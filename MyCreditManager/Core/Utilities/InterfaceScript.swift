...
@frozen enum InterfaceScript {
    ...
    @frozen enum WrongInput: Error, CustomDebugStringConvertible {
        ...
        case subjectNotExist(name: String, subject: Subject)
        case statusNotFound
        ...
        var debugDescription: String {
            ...
            case .subjectNotExist(let name, let subject):
                return "\(name) 학생의 \(subject.title) 과목의 성적은 존재하지 않습니다."
            case .statusNotFound:
    ...
    @frozen enum AdjustScore {
        case readLineScore
        case adjustScoreSuccess(name: String, subject: Subject)
        
        var description: String {
            switch self {
            case .readLineScore:
                return "성적을 추가할 학생의 이름, 과목 이름, 성적(A+, A, F 등)을 띄어쓰기로 구분하여 차례로 작성해주세요.\n입력예) Mickey Swift A+\n만약에 학생의 성적 중 해당 과목이 존재하면 기존 점수가 갱신됩니다."
            case .adjustScoreSuccess(let name, let subject):
                return "\(name) 학생의 \(subject.title) 과목이 \(subject.grade)로 추가(변경)되었습니다."
            }
        }
    }
    ...
    @frozen enum RemoveScore {
        case readLineScore
        case removeScoreSuccess(name: String, subject: Subject)
        ...
        var description: String {
            ...
            case .removeScoreSuccess(let name, let subject):
                return "\(name) 학생의 \(subject.title) 과목의 성적이 삭제되었습니다."
            }
        }
    }
